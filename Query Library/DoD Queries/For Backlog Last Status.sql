...
SELECT
  ...
  t.name AS transfer_status_name,
  (o.transfer_status_updated_at + INTERVAL '6 hours') AS transfer_status_updated_at_bd,

  -- Delivery hub info
  o.delivery_hub_id,
  dh.name AS delivery_hub_name,
  COALESCE(hzm.zone_type, 'OSD') AS delivery_zone

FROM orders AS o
LEFT JOIN transfer_statuses AS t
  ON t.id = o.transfer_status_id
LEFT JOIN hubs AS dh
  ON dh.id = o.delivery_hub_id
LEFT JOIN hub_zone_map AS hzm
  ON hzm.hub_id = dh.id

WHERE o.consignment_id IN (
  'R0XXXXXXX1',
  'R0XXXXXXX2'
  -- add your consignment IDs here
);
