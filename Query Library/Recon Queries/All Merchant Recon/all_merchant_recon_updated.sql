/* ============================================================
   Pricing Export for Google Sheet Reconciliation (consignment-wise)
   - NO timestamp filters (pure consignment lookup)
   - NO hub exclusions
   - Zone Type logic aligned with your doc + override city-match:
       1) Same hub => Same City
       2) ISD -> SUB:
            is_sub_area TRUE  => ISD to Sub
            is_sub_area FALSE => ISD to OSD
          (this MUST be checked before city-match)
       3) SUB -> ISD:
            is_sub_area FALSE => OSD to ISD
            is_sub_area TRUE  => OSD to OSD
          (this MUST be checked before city-match)
       4) Same City other rules as per doc (ISD->ISD & non-sub, then city-match)
============================================================ */
...
hub_zone_map AS (
  SELECT
    h.id AS hub_id,
    CASE
      WHEN h.id IN (1,2,3,4,5,6,7,8,9,73,92,145,172) THEN 'ISD'
...
SELECT
  ...
  /* ===== status/timestamps (shown, not filtered) ===== */
...
CROSS JOIN LATERAL (
  SELECT
    CASE
      ...
      /* 2) ISD -> SUB (per doc, depends on is_sub_area) — must override city-match */
      WHEN norm.pz = 'ISD' AND norm.dz = 'SUB'
        THEN CASE WHEN norm.is_sub THEN 'ISD to Sub' ELSE 'ISD to OSD' END

      /* 3) SUB -> ISD — must override city-match */
      WHEN norm.pz = 'SUB' AND norm.dz = 'ISD'
        THEN CASE WHEN norm.is_sub THEN 'OSD to OSD' ELSE 'OSD to ISD' END

      /* 4) Same City rule (per doc): ISD -> ISD and NOT sub-area */
      WHEN norm.pz = 'ISD' AND norm.dz = 'ISD' AND norm.is_sub = FALSE
        THEN 'Same City'

      /* 5) City-match fallback (kept AFTER overrides) */
...
CROSS JOIN LATERAL (
  SELECT
    CASE
      WHEN rt.zone_type = 'ISD to Sub' THEN 'SUB'
      WHEN rt.zone_type IN ('ISD to OSD','OSD to OSD') THEN 'OSD'
      WHEN norm.dz = 'ISD' AND norm.is_sub = FALSE THEN 'ISD'
      ELSE 'OSD'
    END AS cod_bucket
) codr
...
WHERE
  o.consignment_id::text = ANY(p.consignment_ids)
